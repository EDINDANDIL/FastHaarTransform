...
/**
 * Вспомогательная программа для построения иллюстраций к практической части.
 *
 * <p>Класс создает искусственный дискретный сигнал со ступенчатой структурой,
 * небольшими колебаниями, шумом и отдельными выбросами. Затем к сигналу
 * применяется быстрое преобразование Хаара. После этого часть коэффициентов
 * зануляется, и по оставшимся коэффициентам выполняется обратное преобразование.
 * Так получается грубая аппроксимация сигнала, показывающая, что первые
 * коэффициенты сохраняют общую структуру, а мелкие детали сглаживаются.</p>
 *
 * <p>В результате запуска создаются PNG-изображения: исходный сигнал,
 * аппроксимация по первым коэффициентам и полное восстановление после обратного
